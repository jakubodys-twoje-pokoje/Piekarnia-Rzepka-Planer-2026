...
const AdminReports: React.FC = () => {
  ...
  useEffect(() => { fetchData(); }, [selectedDate, viewScope]);
    ...
    <div className="space-y-8 pb-20">
      {editModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-200">
             <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Korekta Raportu</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{editModal.location_name} &middot; {editModal.date}</p>
                </div>
                <button onClick={() => setEditModal(null)} className="p-3 bg-slate-50 rounded-full"><X size={24}/></button>
             </div>
             <form onSubmit={handleUpdate} className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-amber-50 rounded-2xl border border-amber-100 space-y-4">
                    <p className="text-xs font-black text-amber-700 uppercase tracking-widest">🍞 Piekarnia</p>
                    <input type="number" step="0.01" value={editModal.bakery_sales} onChange={e => setEditModal({...editModal, bakery_sales: parseFloat(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold outline-none" placeholder="Sprzedaż" />
                    <input type="number" step="0.01" value={editModal.bakery_loss} onChange={e => setEditModal({...editModal, bakery_loss: parseFloat(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold outline-none" placeholder="Strata" />
                  </div>
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                    <p className="text-xs font-black text-slate-700 uppercase tracking-widest">🍰 Cukiernia</p>
                    <input type="number" step="0.01" value={editModal.pastry_sales} onChange={e => setEditModal({...editModal, pastry_sales: parseFloat(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold outline-none" placeholder="Sprzedaż" />
                    <input type="number" step="0.01" value={editModal.pastry_loss} onChange={e => setEditModal({...editModal, pastry_loss: parseFloat(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold outline-none" placeholder="Strata" />
                  </div>
                </div>
                <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl">Zatwierdź korektę</button>
             </form>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
           ...
           <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Widok Administratora</p>
              <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Dziennik raportów</h2>
           </div>
        </div>
        ...
        <div className="overflow-x-auto">
                ...
                <tr key={report.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-7">
                    <div className="flex flex-col">
                       <span className="text-sm font-black text-slate-900">{report.bakery_sales.toLocaleString()} zł</span>
                       <span className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">-{report.bakery_loss} zł</span>
                    </div>
                  </td>
                  <td className="px-6 py-7">
                    <div className="flex flex-col">
                       <span className="text-sm font-black text-slate-900">{report.pastry_sales.toLocaleString()} zł</span>
                       <span className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">-{report.pastry_loss} zł</span>
                    </div>
                  </td>
                  <td className="px-6 py-7">
          ...
          {reports.length === 0 && !loading && (
            <div className="py-24 flex flex-col items-center justify-center text-slate-300">
               ...
               <p className="text-xs font-black uppercase tracking-widest">Brak danych na ten dzień</p>
            </div>
          )}
        </div>
      </div>
