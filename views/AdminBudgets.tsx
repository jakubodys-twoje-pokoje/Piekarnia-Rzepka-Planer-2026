...
// Helper: Wylicza numery tygodni ISO dla danego miesiąca i roku
const getWeeksInMonth = (month: number, year: number) => {
  ...
  // Sprawdź ostatni dzień na wszelki wypadek
...
const AdminBudgets: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Transformacja danych do mapy [location_id-week_number]
  ...
  return (
    <div className="space-y-8 pb-24">
      ...
      <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none">Budżetowanie Sieci</h1>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              {saving ? <Loader2 size={18} className="animate-spin" /> : (saveStatus === 'success' ? <CheckCircle2 size={18}/> : <Save size={18}/>)}
      ...
      {/* Main Matrix Table */}
      <div className="bg-white rounded-[3.5rem] border border-slate-200 shadow-sm overflow-hidden overflow-x-auto custom-scrollbar">
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="sticky left-0 z-20 bg-slate-900 px-10 py-8 text-left border-r border-white/5 min-w-[300px]">
              ...
              {activeWeeks.map(w => (
                <th key={w} className="px-6 py-8 text-center min-w-[280px] border-r border-white/5">
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500 mb-1">Tydzień ISO</p>
                  <p className="text-2xl font-black tracking-tight">{w}</p>
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {locations.map(loc => (
              <tr key={loc.id} className="hover:bg-slate-50/50 transition-colors group">
                {/* Location Name */}
                <td className="sticky left-0 z-10 bg-white group-hover:bg-slate-50 px-10 py-10 border-r border-slate-100 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.05)]">
                  <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight leading-none mb-1">{loc.name}</h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{loc.address}</p>
                </td>
                    ...
                    <td key={w} className="p-4 border-r border-slate-100 align-top">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="space-y-2">
                          <p className="text-[8px] font-black text-amber-600 uppercase tracking-widest ml-1">Piekarnia</p>
                          <div className="bg-amber-50/30 p-3 rounded-2xl border border-amber-100/50 space-y-3">
                            <div>
                              <p className="text-[7px] font-bold text-slate-400 uppercase mb-1">Przychód</p>
                              <input 
                                type="number"
                                value={data.bakery_daily_target || ''}
                                onChange={e => handleUpdate(loc.id, w, 'bakery_daily_target', e.target.value)}
                                placeholder="0"
                                className="w-full bg-transparent font-black text-sm text-slate-800 outline-none p-0 border-none focus:ring-0"
                              />
                            </div>
                            <div className="pt-2 border-t border-amber-100/50 flex items-center justify-between">
                              <div>
                                <p className="text-[7px] font-bold text-slate-400 uppercase mb-1">Strata %</p>
                                <input 
                                  type="number"
                                  value={data.bakery_loss_target || ''}
                                  onChange={e => handleUpdate(loc.id, w, 'bakery_loss_target', e.target.value)}
                                  placeholder="5"
                                  className="w-full bg-transparent font-black text-sm text-amber-600 outline-none p-0 border-none focus:ring-0"
                                />
                              </div>
                              <TrendingDown size={14} className="text-amber-200" />
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Pastry Column */}
                        <div className="space-y-2">
                          <p className="text-[8px] font-black text-pink-600 uppercase tracking-widest ml-1">Cukiernia</p>
                          <div className="bg-pink-50/30 p-3 rounded-2xl border border-pink-100/50 space-y-3">
                            <div>
                              <p className="text-[7px] font-bold text-slate-400 uppercase mb-1">Przychód</p>
                              <input 
                                type="number"
                            ...
                            <div className="pt-2 border-t border-pink-100/50 flex items-center justify-between">
                              <div>
                                <p className="text-[7px] font-bold text-slate-400 uppercase mb-1">Strata %</p>
                                <input 
                                  type="number"
                                  ...
                                  placeholder="5"
                                  className="w-full bg-transparent font-black text-sm text-pink-600 outline-none p-0 border-none focus:ring-0"
                                />
                              </div>
                              <TrendingDown size={14} className="text-pink-200" />
                            </div>
                          </div>
                        </div>
                      </div>
        ...
        {locations.length === 0 && (
          <div className="py-40 flex flex-col items-center justify-center gap-6">
            ...
            <p className="text-xs font-black text-slate-300 uppercase tracking-[0.3em]">Brak lokalizacji do wyświetlenia</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-6 px-10">
        <div className="flex items-center gap-2">
           ...
           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Sekcja Piekarnia</span>
        </div>
        <div className="flex items-center gap-2">
           <div className="w-3 h-3 rounded bg-pink-500"></div>
           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Sekcja Cukiernia</span>
        </div>
        <div className="flex items-center gap-2 text-slate-300">
           <AlertCircle size={14} />
           <span className="text-[9px] font-bold uppercase tracking-widest">Wartości strat domyślnie interpretowane jako % sprzedaży</span>
        </div>
      </div>
    </div>
  );
