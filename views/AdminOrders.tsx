...
import { 
  ...
  Table as TableIcon, List, MapPin, Info, ArrowRightLeft
} from 'lucide-react';
    ...
    <div className="space-y-8 pb-24 max-w-[1600px] mx-auto">
      <style>{`
        @media print {
          ...
          th, td { border: 1px solid black !important; padding: 4px 2px !important; font-size: 8pt !important; color: black !important; font-weight: bold !important; }
          th { background: #f0f0f0 !important; }
          .section-row td { background: #e0e0e0 !important; text-align: center !important; }
        }
      `}</style>
      ...
      <div className="printable-area bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-sm">
        <div className="mb-10 flex items-center justify-between">
           <div>
             ...
             <p className="text-[10px] font-black text-amber-600 uppercase tracking-[0.4em] mt-2">Arkusze Wyjazdowe i Produkcyjne</p>
           </div>
           <div className="no-print hidden lg:flex items-center gap-4 text-slate-400">
              <Info size={16} />
              <p className="text-[9px] font-bold uppercase tracking-widest">Użyj poziomego druku dla macierzy.</p>
           </div>
        </div>
        ...
        {viewMode === 'summary' ? (
          ...
          /* MATRIX VIEW WITH DUAL SCROLLBAR */
            ...
            <div className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm print:border-black print:rounded-none">
              ...
              >
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-white print:bg-slate-200 print:text-black">
                      <th className="sticky left-0 z-20 bg-slate-900 px-6 py-8 text-left text-[10px] font-black uppercase tracking-widest border-r border-white/10 print:bg-white print:text-black print:border-black min-w-[220px]">Towar / Punkt</th>
                      {locations.map(l => (
                        <th key={l.id} className="px-3 py-8 text-center text-[10px] font-black uppercase tracking-widest border-r border-white/5 min-w-[120px] print:border-black">
                          <div className="flex flex-col items-center gap-1">
                            <MapPin size={12} className="text-amber-500 mb-1 print:hidden" />
                            <span className="leading-tight">{l.name.replace('Piekarnia ', '')}</span>
                          </div>
                        </th>
                      ))}
                      <th className="sticky right-0 z-20 bg-amber-500 px-6 py-8 text-center text-[10px] font-black uppercase tracking-widest text-white print:bg-white print:text-black print:border-black min-w-[100px]">SUMA</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 print:divide-black">
                    {['Piekarnia', 'Cukiernia'].map(section => (
                      <React.Fragment key={section}>
                        ...
                        {productionSummary.filter(s => s.section === section).map((prod, idx) => (
                          <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                            <td className="sticky left-0 z-10 bg-white px-6 py-4 text-[11px] font-black text-slate-800 border-r border-slate-100 group-hover:bg-slate-50 print:border-black">
                            ...
                            {locations.map(loc => {
                              ...
                              return (
                                <td key={loc.id} className={`px-2 py-4 text-center text-lg font-black border-r border-slate-50 print:border-black ${itemInOrder ? 'text-slate-900 bg-amber-50/20' : 'text-slate-100 print:text-transparent'}`}>
                                  {itemInOrder?.quantity || ''}
                                </td>
                              );
                            })}
                            <td className="sticky right-0 z-10 bg-amber-50/90 px-6 py-4 text-center text-xl font-black text-amber-700 group-hover:bg-amber-100 print:bg-white print:text-black print:border-black">
                               {prod.qty}
                            </td>
                          </tr>
                        ))}
