...
import { Send, Globe, History, User, Loader2, AlertCircle } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form onSubmit={handleSend} className="space-y-4">
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold text-sm"
                >
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-medium text-sm"
                ></textarea>
              </div>
              <button 
                type="submit"
                disabled={sending}
                className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-amber-600 transition-all shadow-xl disabled:opacity-50"
              >
      ...
      <div className={`${isAdmin ? 'lg:col-span-2' : 'lg:col-span-3'} space-y-6`}>
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-black text-slate-800">Ostatnie Wiadomości</h2>
              ...
              <div key={msg.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-black text-slate-900 uppercase">System Zarządzania</p>
                      <p className="text-[9px] font-bold text-slate-400">{new Date(msg.created_at).toLocaleString()}</p>
                    </div>
                  </div>
