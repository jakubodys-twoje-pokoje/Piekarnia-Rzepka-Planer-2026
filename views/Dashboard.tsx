  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Krytyczne sprawdzenie: czy location_id to poprawny UUID?
        if (!isAdmin && user.default_location_id && user.default_location_id.length > 5) {
          query = query.eq('location_id', user.default_location_id);
        } else if (!isAdmin) {
          // Jeśli nie mamy UUID, nie wykonujemy zapytania z filtrem, aby uniknąć 400
          console.warn("Próba zapytania z niepoprawnym ID lokalizacji.");
          setLoading(false);
          return;
        }
        ...
        const totals = (reports || []).reduce((acc, curr) => ({
          sales: acc.sales + (curr.bakery_sales || 0) + (curr.pastry_sales || 0),
          bakery: acc.bakery + (curr.bakery_sales || 0),
          pastry: acc.pastry + (curr.pastry_sales || 0),
          loss: acc.loss + (curr.bakery_loss || 0) + (curr.pastry_loss || 0),
          bakeryLoss: acc.bakeryLoss + (curr.bakery_loss || 0),
          pastryLoss: acc.pastryLoss + (curr.pastry_loss || 0),
        ...
        setStats({
          ...
          planExecution: 0 // Tu w przyszłości pobierzemy targets
        });
  ...
  return (
    <div className="space-y-8 pb-12 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
         ...
         <div className="relative z-10">
            ...
            <p className="text-slate-500 font-medium mt-1">
              {isAdmin ? 'Monitorujesz wyniki całej sieci.' : `Wyniki dla: ${locationName}.`}
            </p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Sprzedaż Netto', value: stats.monthlySales.toLocaleString(), unit: 'zł', icon: <TrendingUp />, color: 'text-emerald-600' },
          { label: 'Łączne Straty', value: stats.monthlyLoss.toLocaleString(), unit: 'zł', icon: <TrendingDown />, color: 'text-rose-600' },
          { label: 'Wykonanie Planu', value: stats.planExecution, unit: '%', icon: <Target />, color: 'text-amber-600' },
          { label: 'Ranking', value: isAdmin ? 'Aktywna' : 'TOP 3', unit: '', icon: <Award />, color: 'text-indigo-600' },
        ].map((kpi, i) => (
              ...
              {calendarData.map((d) => (
                <button
                  ...
                  className={`aspect-square rounded-[1.25rem] border-2 flex flex-col items-center justify-center transition-all ${
                    d.status === 'perfect' ? 'bg-emerald-50 border-emerald-100 text-emerald-700 hover:scale-105 shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-300'
                  }`}
                >
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {messages.length > 0 ? messages.map((msg) => (
              ...
              <p className="text-xs text-slate-400 font-bold uppercase text-center py-10">Brak nowych ogłoszeń</p>
            )}
          </div>
        </div>
      </div>
